...
app = FastAPI(title="Documentacion de API-Licencias", description="La API gestiona las licencias registradas en el sistema", version="1.0.0")
...
@app.post("/licencias",summary="Registrar una nueva licencia")
...
@app.post("/vigencias/", summary="Agregar una nueva vigencia",status_code=201)
def agregar_vigencia(nueva_vigencia: schemas.Vigencia, mydb=Depends(get_db_connection)):
    cursor = mydb.cursor()
...
@app.post("/estados/", summary="Agregar un nuevo estado", status_code=201)
def agregar_estado(nuevo_estado: schemas.Estado, mydb=Depends(get_db_connection)):
    cursor = mydb.cursor()
...
@app.patch("/licencias/{licencia_id}", summary="Actualizar la informacion de una licencia")
def actualizar_licencia(licencia_id: int, licencia: schemas.LicenciaUpdate, db=Depends(get_db_connection)):
...
@app.patch("/vigencias/{vigencia_id}",summary="Actualizar la informacion de una vigencia")
def actualizar_vigencia(vigencia_id: int, vigencia: schemas.VigenciaUpdate, db=Depends(get_db_connection)):
...
@app.patch("/estados/{estado_id}",summary="Actualizar la informacion de un estado")
def actualizar_estado(estado_id: int, estado: schemas.EstadoUpdate, db=Depends(get_db_connection)):
...
# Endpoint para consultar una licencia por numero de licencia
@app.get("/licencias/{numero}",summary="Consultar una licencia por numero de licencia",response_model=schemas.ConsultaLicencia)
...
@app.get("/licencias/{licencia_id}/vigencias",summary="Obtener todas las vigencias de una licencia", response_model=List[schemas.VigenciaResponse])
...
@app.get("/vigencias/{vigencia_id}/estados", summary="Obtener todas los estados de una vigencia",response_model=List[schemas.EstadoResponse])
...
@app.get("/licencias/conductor/{conductor_name}", summary="Obtener todas las licencias de un conductor", response_model=List[schemas.LicenciaConFechaExpiracion])
...
@app.delete("/licencias/{licencia_id}", summary="Eliminar una licencia por ID. Se eliminar todos los registros de tablas relacionadas")
